...
function main() {
  ...
  document.body.appendChild(renderer.domElement); // Track mouse coordinates, as normalized device coordinates
  // (for ray-casting)
  ...
  window.addEventListener('mousemove', onMouseMove, false); // Make sure to resize renderer on browser resize.
  ...
  sceneManager = new SceneManager(renderer); // Hook up the back button to bring us back to the globe scene
  ...
  ui.setMode(ui.UIMode.Globe); // start the update loop
