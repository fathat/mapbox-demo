...
import { accessToken } from "./token"; // missing a .d.ts for this module so use the old school require syntax

const TileBelt = require('@mapbox/tilebelt'); // represents a point on the globe, with an integer zoom level (between 0-15)
...
export function sphereUVtoLatLon(uv) {
...
export function locationToUVbox(location) {
...
export function loadTileFromLatLonIntoImage(location, targetImage) {
  const tile = TileBelt.pointToTile(location.lon, location.lat, location.zoom);
  const [tx, ty, tz] = tile;
