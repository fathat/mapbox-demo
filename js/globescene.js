...
import * as ui from "./uielements"; // This scene allows you to rotate around a globe and pick tiles
...
export class GlobeScene {
  ...
  constructor(renderer) {
    ...
    this.camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.05, 1000); // start loading earth model
    ...
    const skybox = cubeLoader.load(['assets/skybox/right.jpg', 'assets/skybox/left.jpg', 'assets/skybox/top.jpg', 'assets/skybox/bottom.jpg', 'assets/skybox/front.jpg', 'assets/skybox/back.jpg']);
    this.scene.background = skybox;
    this.ambientLight = new THREE.AmbientLight(0xffffff, 0.75);
    this.scene.add(this.ambientLight);
    this.directionalLight = new THREE.DirectionalLight(0xffffff, 1.5);
    this.scene.add(this.directionalLight); // add a sphere for the atmosphere halo & selection grid
    ...
    ui.inputZoom.addEventListener('change', () => {
    ...
    renderer.domElement.addEventListener('mousemove', ev => {
      if (Math.abs(ev.clientX - dragStart.x) > 5 || Math.abs(ev.clientY - dragStart.y) > 5) {
        drag = true;
      }
    });
    renderer.domElement.addEventListener('touchmove', ev => {
      if (Math.abs(ev.touches[0].clientX - dragStart.x) > 5 || Math.abs(ev.touches[0].clientY - dragStart.y) > 5) {
        drag = true;
      }
    });
